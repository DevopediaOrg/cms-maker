...
class HtmlScraper(object):
    ...
    sitemap = {
        ...
        'www.designnews.com': {'Image': ('class_', 'docimage', 'src'),
                               'Author': ('class_', 'gray', 'author_section.b.text.split()[0]')},
        ...
        'www.greenbaypressgazette.com': {
            'Author': ('class', 'asset-metabar-author asset-metabar-item', 'author[0].get_text().strip()')},
        'www.zdnet.com': {'Author': ('rel', 'author', 'author[0].get_text().strip()')},
        'uk.businessinsider.com': {'Author': ('rel', 'author', 'author[0].get_text().strip()')},
        'uk.citizen-times.com': {
            'Author': ('class', 'asset-metabar-author asset-metabar-item', 'author[0].a.get_text().strip()')},
        ...
        for key in item:
            if withText or not withText and key != 'FullText':
                try:  # re.search(r'^(FirstText|FullText|Title)$',key)
                    if item[key] and key != "CANDIDATE_AUTHORS":
    ...
    def __scrape_article(self, article, withText=True):
        ...
        if withText:
            ...
            item = copy.deepcopy(news_item)
            self.news_items.append(item)
        item["CANDIDATE_AUTHORS"] = candidate_authors
        ...
        for key in parsely_keys:
            if key in content_dict:
                if key == 'link' or key == 'image_url':
                    ...
                    news_item[parsely_keys[key]] = self.__date(
                        struct=iso8601.parse_date(content_dict[key].decode('utf-8')).timetuple())
    ...
    def __process_metadata(self, soup):
        ...
        for tag in soup.find_all('meta'):
            ...
            if (('content' in tag.attrs and 'name' in tag.attrs and tag.attrs['name'] == 'description')
                    or ('content' in tag.attrs and 'property' in tag.attrs and tag.attrs[
